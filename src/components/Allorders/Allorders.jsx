        ...
        <>
          ...
          <div className="row justify-content-between g-3  ">
            {userOrders.map((order , idx)=>{
             return <div key={idx} className="col-md-5 bg-main-light rounded-4 p-3 ">
                <span className='h6 me-3 text-main'>total cost: {order.totalOrderPrice} EGP</span>
                <span className='h6  text-main'>payment method: {order.paymentMethodType}</span>
                <p className='m-0 mt-3'>phone: {order.shippingAddress.phone}</p>
                <p className='m-0 '>city: {order.shippingAddress.city}</p>
                <div className='row'>
                  {order.cartItems.map((item , index)=>{
                    return <div key={index} className=' col-md-5 bg-main text-white rounded-3 p-3 m-2 '>
